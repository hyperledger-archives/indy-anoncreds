...
class Issuer:
    ...
    async def genClaimDef(self, name, version, attrNames, type=TYPE_CL) -> ClaimDefinition:
    ...
    async def genKeys(self, id: ID, p_prime=None, q_prime=None):
        ...
        await self.wallet.submitPublicKeys(id=id, pk=pk, pkR=pkR)
        await self.wallet.submitSecretKeys(id=id, sk=sk, skR=skR)

    async def issueAccumulator(self, id: ID, iA, L):
        accum, tails, accPK, accSK = await self._nonRevocationIssuer.issueAccumulator(id, iA, L)
        await self.wallet.submitAccumPublic(id=id, accumPK=accPK, accum=accum, tails=tails)
    ...
    async def issueClaim(self, id: ID, claimRequest: ClaimRequest, iA=None, i=None) -> Claims:
        ...
        c2 = await self._issueNonRevocationClaim(id, claimRequest.Ur, iA, i) if claimRequest.Ur else None
    ...
    async def _issueNonRevocationClaim(self, id: ID, Ur, iA=None, i=None) -> NonRevocationClaim:
        claim, accum, ts = await self._nonRevocationIssuer.issueNonRevocationClaim(id, Ur, iA, i)
